import type { JSONSchema7, TemplateDiagnostic, AnalysisResult } from "./types.ts";
import {
  parse,
  isSingleExpression,
  extractPathSegments,
...
import { resolveSchemaPath, resolveArrayItems, simplifySchema } from "./schema-resolver.ts";
 ...
 */
export function analyze(template: string, inputSchema: JSONSchema7): AnalysisResult {
 ...
 */
function validateStatement(stmt: hbs.AST.Statement, ctx: AnalysisContext): void {
  switch (stmt.type) {
    ...
    default:
      ...
      addDiagnostic(ctx, "warning", `Unsupported AST node type: "${stmt.type}"`, stmt);
  }
}
 ...
 */
function validateMustache(stmt: hbs.AST.MustacheStatement, ctx: AnalysisContext): void {
  ...
  if (stmt.path.type === "SubExpression") {
    addDiagnostic(ctx, "warning", "Sub-expressions are not statically analyzable", stmt);
    return;
  }
 ...
 */
function getBlockArgument(stmt: hbs.AST.BlockStatement): hbs.AST.Expression | undefined {
 ...
 */
function validateBlock(stmt: hbs.AST.BlockStatement, ctx: AnalysisContext): void {
  ...
  switch (helperName) {
    ...
    default:
      addDiagnostic(ctx, "warning", `Unknown block helper "{{#${helperName}}}" — cannot analyze statically`, stmt);
 ...
 */
function validateIfBlock(stmt: hbs.AST.BlockStatement, ctx: AnalysisContext): void {
  ...
  if (arg) {
    ...
    addDiagnostic(ctx, "error", `"{{#${getBlockHelperName(stmt)}}}" requires an argument`, stmt);
  }
 ...
 */
function validateEachBlock(stmt: hbs.AST.BlockStatement, ctx: AnalysisContext): void {
  const arg = getBlockArgument(stmt);
  if (!arg) {
 ...
 */
function validateWithBlock(stmt: hbs.AST.BlockStatement, ctx: AnalysisContext): void {
  const arg = getBlockArgument(stmt);
  if (!arg) {
...
/**
 * Infère le type d'une expression en la résolvant dans le schema courant.
 * Retourne un schema vide (unknown) en fallback si le chemin n'est pas résolvable.
 */
  ...
  switch (helperName) {
    ...
    case "with": {
      ...
      if (arg) {
        ...
        if (innerSchema) {
          return inferProgramType(stmt.program, { ...ctx, current: innerSchema });
        }
      }
...
function resolveExpressionWithDiagnostics(
  ...
  // Résoudre le chemin segment par segment dans le schema courant
  const resolved = resolveSchemaPath(ctx.current, segments);
