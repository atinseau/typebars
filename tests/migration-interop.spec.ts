...
import { execute, analyze, parse, TemplateEngine } from "../src/index.ts";
import { isSingleExpression, extractPathSegments, getEffectivelySingleExpression } from "../src/parser.ts";
...
describe("Migration: normalizeTemplateTokens → Handlebars normalisation native", () => {
  ...
  test("Non-formatted string with extra whitespace is equivalent", () => {
    ...
    const unformatted = execute("coucou {{       meetingId       }}", { meetingId: "value" });
  ...
  test("Multi-template with whitespace normalizes identically", () => {
    ...
    const messy = execute("{{       accountIds       }} coucou {{       meetingId       }}", {
      accountIds: "A",
      meetingId: "B",
    });
...
describe("Migration: Template identifiers ({{key:id}}) — LIMITATIONS", () => {
  ...
  test("Workaround: single identifier with namespace preserves type", () => {
    ...
    const result = execute("salut {{node_0.meetingId}} ok {{node_0.meetingId}}", data);
...
describe("Migration: TemplateEngine strict mode", () => {
  ...
  test("Execute succeeds with valid schema", () => {
    ...
    const result = engine.execute("{{accountId}}", { accountId: "123" }, schema);
    expect(result).toBe("123");
  });
  ...
  test("Execute throws with invalid schema reference", () => {
    ...
    expect(() => engine.execute("{{accountId}}", { accountId: "123" }, schema)).toThrow();
  });
