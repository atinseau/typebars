...
import { resolveSchemaPath, resolveArrayItems, simplifySchema } from "../src/index.ts";
...
describe("schema-resolver", () => {
  describe("resolveSchemaPath", () => {
    ...
    test("retourne undefined pour un chemin profond inexistant", () => {
      expect(resolveSchemaPath(userSchema, ["address", "country"])).toBeUndefined();
    });
  ...
  describe("simplifySchema", () => {
    test("déplie un oneOf à un seul élément", () => {
      expect(simplifySchema({ oneOf: [{ type: "string" }] })).toEqual({ type: "string" });
    });

    test("déplie un anyOf à un seul élément", () => {
      expect(simplifySchema({ anyOf: [{ type: "number" }] })).toEqual({ type: "number" });
    });

    test("déplie un allOf à un seul élément", () => {
      expect(simplifySchema({ allOf: [{ type: "boolean" }] })).toEqual({ type: "boolean" });
    });
