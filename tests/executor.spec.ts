...
describe("executor", () => {
  ...
  describe("bloc #if / #unless", () => {
    test("#if truthy → branche principale", () => {
      expect(execute("{{#if active}}yes{{else}}no{{/if}}", userData)).toBe("yes");
    });

    test("#if falsy → branche else", () => {
      expect(execute("{{#if active}}yes{{else}}no{{/if}}", { active: false })).toBe("no");
    });
    ...
    test("#if avec expression dans le corps", () => {
      expect(execute("{{#if active}}Hello {{name}}{{/if}}", userData)).toBe("Hello Alice");
    });

    test("#unless truthy → branche else", () => {
      expect(execute("{{#unless active}}no{{else}}yes{{/unless}}", userData)).toBe("yes");
    });

    test("#unless falsy → branche principale", () => {
      expect(execute("{{#unless active}}no{{else}}yes{{/unless}}", { active: false })).toBe("no");
    });
  });
  ...
  describe("coercion de type — bloc unique", () => {
    ...
    test("#if avec littéraux numériques inline", () => {
      ...
      expect(execute("{{#if active}}42{{else}}0{{/if}}", { active: false })).toBe(0);
    });

    test("#if avec littéral décimal → retourne un number décimal", () => {
      expect(execute("{{#if active}}3.14{{else}}2.71{{/if}}", userData)).toBe(3.14);
    });
    ...
    test("#if avec littéral booléen true → retourne un boolean", () => {
      const result = execute("{{#if active}}true{{else}}false{{/if}}", userData);
      expect(result).toBe(true);
      expect(typeof result).toBe("boolean");
    });

    test("#if avec littéral booléen false → retourne un boolean", () => {
      const result = execute("{{#if active}}true{{else}}false{{/if}}", { active: false });
      expect(result).toBe(false);
      expect(typeof result).toBe("boolean");
    });

    test("#if avec littéral null → retourne null", () => {
      const result = execute("{{#if active}}null{{else}}fallback{{/if}}", userData);
      expect(result).toBeNull();
    });

    test("#if avec texte non-littéral → retourne une string brute", () => {
      const result = execute("{{#if active}}hello{{else}}world{{/if}}", userData);
  ...
  describe("bloc #each", () => {
    ...
    test("#each avec branche else (tableau vide)", () => {
      const result = execute("{{#each items}}{{this}}{{else}}empty{{/each}}", { items: [] });
  ...
  describe("bloc #with", () => {
    test("#with change le contexte", () => {
      expect(execute("{{#with address}}{{city}}, {{zip}}{{/with}}", userData)).toBe("Paris, 75001");
    });

    test("#with imbriqué", () => {
      const data = { a: { b: { c: "deep" } } };
      expect(execute("{{#with a}}{{#with b}}{{c}}{{/with}}{{/with}}", data)).toBe("deep");
    });
  });
  ...
  describe("combinaisons complexes", () => {
    ...
    test("texte + expression + #if + #each", () => {
      ...
      expect(result).toBe("User: Alice | Tags: developer, typescript, open-source");
    });
  });
});
