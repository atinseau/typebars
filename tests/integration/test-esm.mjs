// ─── ESM Import Test (Node.js) ──────────────────────────────────────────────
// This script is executed by Node.js to verify that the ESM build
...
if (errors.length > 0) {
	console.error(`[ESM/Node] ${errors.length} assertion(s) failed:`);
	for (const e of errors) {
		console.error(`  ${e}`);
	}
	process.exit(1);
} else {
	console.log("[ESM/Node] All assertions passed ✓");
}
